...
def shift_row(state):
    new_state = [state[0], state[5], state[10], state[15],
...
def mix_column(state):
    new_state = []
    for byte_set in range(0, 16, 4):
        new_state += galois_multiply(state[byte_set:byte_set+4], [0x02, 0x03, 0x01, 0x01])

    return new_state

def encrypt_block(iv, block, key):
    curr_state = [i ^ b for i,b in zip(iv, block)]

    initial_key_segment, full_round_keys, final_key_segment = (key[0:16], key[16:len(key)-16], key[len(key)-16: len(key)])
    ...
    for key_segment in full_round_keys:
        curr_state = add_round_key(mix_column(shift_row(byte_sub(curr_state))), key_segment)
...
def ibyte_sub(state):
    new_state = [isbox[b] for b in state]
    return new_state

def ishift_row(state):
    new_state = [state[0], state[13], state[10], state[7],
...
def imix_column(state):
    new_state = []
    for byte_set in range(0, 16, 4):
        new_state += galois_multiply(state[byte_set:byte_set+4], [0x0E, 0x0B, 0x0D, 0x09])

    return new_state

def decrypt_block(iv, block, key):
    initial_key_segment, full_round_keys, final_key_segment = (key[0:16, key[16:len(key)-17], key[len(key-17): len(key)-1]])
